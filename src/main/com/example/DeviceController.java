...
public class DeviceController {
	...
	private String getSpecValue(File apk, String target) {
		List<String> result = Utility.cmd("getSpecValue", CoreOptions.PYTHON, CoreOptions.APK_INFO_GETTER_DIR, apk.getAbsolutePath(),
				target);
	...
	public void turnOffBluetooth(Device phone) throws IOException, InterruptedException {
		...
		Utility.cmd("turnOffBluetooth", CoreOptions.PYTHON, CoreOptions.TURN_OFF_BLUETOOTH_DIR, phone.getSerialNum());
	}
	...
	public void turnOnBluetooth(Device phone) throws IOException, InterruptedException {
		...
		Utility.cmd("turnOnBluetooth", CoreOptions.PYTHON, CoreOptions.TURN_ON_BLUETOOTH_DIR, phone.getSerialNum());
	}
	...
	public void clearWearGms(Device wear) throws IOException, InterruptedException {
		...
		Utility.cmd("clearWearGms", CoreOptions.PYTHON, CoreOptions.CLEAR_GMS_DIR, wear.getSerialNum());
	}

	public void installApk(Device device, String apkPath) {
		System.out.println("【installApk】" + device.getSerialNum());
		Utility.cmd("installApk", CoreOptions.PYTHON, CoreOptions.INSTALL_APK_DIR, device.getSerialNum(), apkPath);
	}

	public void uninstallApk(Device phone, String packageName) {
		System.out.println("【uninstallWearApk】");
		Utility.cmd("uninstallApk", CoreOptions.PYTHON, CoreOptions.SCRIPT_DIR + "\\uninstallApk.py", phone.getSerialNum(),
				packageName);
	}
	
	public List<String> waitWearInstallApp(Device wear, String packageName){
		System.out.println("【waitWearInstallApp】");
		System.out.println("【waitWearInstallApp】package: " + packageName);
		return Utility.cmd("waitWearInstallApp", CoreOptions.PYTHON, CoreOptions.SCRIPT_DIR + "\\waitWearAppInstall.py", wear.getSerialNum(), packageName);
	}
}
